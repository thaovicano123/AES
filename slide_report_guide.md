...
**Nội dung cần có:**
- **Sơ đồ khối AES-256 core** (vẽ FSM + datapath)
- **Thiết kế Verilog:**
  - Ngôn ngữ: Verilog HDL (IEEE 1364-2001)
  - Tổng số files: 4 modules chính
  - Tổng số dòng code: ~2,000 lines RTL
- **Thông số kỹ thuật:**
  - Standard: NIST FIPS-197 AES-256 ECB mode
  - Key size: 256-bit
  - Block size: 128-bit
  - Rounds: 14 rounds
  - Architecture: Iterative FSM (1 round/cycle)
- **Performance:**
  - Latency: ~17-20 cycles/block (~1.33 μs @ 15 MHz)
  - Throughput: ~96 Mbps @ 15 MHz
  - Speedup: 150-250× so với software
- **Tài nguyên sử dụng (sau synthesis):**
  - 12,664 LUTs (21% FPGA)
  - 1,959 Registers (3%)
  - Key expansion: 6,985 LUTs (combinational)
- **Modules Verilog:**
  - `aes256_ahb_wrapper.v`: AHB slave interface (176 lines)
  - `aes256_core.v`: FSM + inline transformations (1,423 lines)
  - `aes256_key_expansion_comb.v`: 15 round keys generation (213 lines)
  - Total: 1,812 lines Verilog code

**Cách trình bày:**
```
┌────────────────────────────────────────────┐
│  AES-256 Verilog RTL Design                │
│                                            │
│  ┌──────────────────────────────────────┐ │
...
│  │  ├─ Inline SubBytes (S-box LUT)      │ │
│  │  ├─ Inline ShiftRows (wire routing)  │ │
│  │  ├─ Inline MixColumns (GF mult)      │ │
│  │  └─ AddRoundKey (XOR)                │ │
│  └──────────────┬───────────────────────┘ │
│                 │                          │
│  ┌──────────────▼───────────────────────┐ │
│  │  aes256_key_expansion_comb.v         │ │
│  │  • Generate 15×128-bit round keys    │ │
│  │  • Fully combinational (0 cycles)    │ │
│  │  • Uses 6,985 LUTs (50% of AES)      │ │
│  └──────────────────────────────────────┘ │
│                                            │
│  Performance: 20 cycles/block, 96 Mbps    │
│  Resources: 12.6K LUTs, 2K Registers       │
└────────────────────────────────────────────┘
```

**Điểm nhấn khi thuyết trình:**
- "Thiết kế bằng Verilog với 1,812 dòng RTL code"
- "Key expansion hoàn toàn tổ hợp - sinh 15 round keys trong 0 chu kỳ"
- "Inline transformations để giảm routing delay"
- "FSM 5 states: IDLE → KEY_ADD → ROUND(×13) → FINAL → DONE"
